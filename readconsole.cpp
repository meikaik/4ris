...
// keyPressed() determines if a key has been pressed
bool keyPressed(){
    //Get key
    int key = getch();
...
ReadConsole::ReadConsole(Score *gameScore, int startLevel, string scriptFile, bool textMode): gameScore{gameScore}, startLevel(startLevel),  scriptFile{scriptFile}, textMode{textMode} {
    ...
    void ReadConsole::startRead(string *file) {
        ...
        string s, numString = "";
        int pos = -1, multipler = 1, len = 0, key;
        
        //Initialize screen only if keymode is active
        if (gameScore->keyMode){
            initscr();
            cbreak();
            nodelay(stdscr, TRUE);
        }
        ...
        for(;;) {
            ...
            //Read command line input if keymode is off
            if (gameScore->keyMode == false){
                if (!(cin >> s)){
                    break;
                }
            }
            
            //Read keypressed if keymode is activated
            if (keyPressed() && gameScore->keyMode){
                cout << getch();
                if (getch() == '\033'){
                    key = getch();
                    key = getch();
    ...
    void ReadConsole::move(int moveCommand) {
        try {
            ...
            gameScore->music.playWav("over.wav", gameScore->bonusMode); //Play gameover sound
